...
$(document).ready(function(){
  ...
  function score_indicate(){
    ...
    let sum = subject_points.reduce((sum,num)=>sum+num) //【追加課題1 reduceにアロー関数で処理】
    // 「合計点：」(id="sum_indicate")に変数「sum」(合計点)を出力させます。
    $("#sum_indicate").text(sum);
        let average = sum / subject_points.length// 変数「average」に平均値を出して代入します。(平均をとりたい数の合計点数(sum) / 全体の個数)
    $("#average_indicate").text(average);
    //教科数が増減すると subject_pointsのlengthも変動するからこれで良い。0点は0点換算で行う。
  ...
  function get_achievement(){
    ...
    if ( averageIndicate >= 80){
      $("#evaluation").css("color","red")
      return "A";
      // もし「averageIndicate」が60以上なら"B"を返します。
    } else if ( averageIndicate >= 60) {
      $("#evaluation").css("color","orange")
      return "B";
      // もし「averageIndicate」が40以上なら"C"を返します。【Cを追記,色変更】
    } else if ( averageIndicate >= 40) {
      $("#evaluation").css("color","blue")
      return "C";
  ...
  function get_pass_or_failure(){
    ...
    return judge //出てきたjudgeをget_pass_or_failureに返す
  };
